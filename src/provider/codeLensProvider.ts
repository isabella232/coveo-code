...
export class CodeLensProvider implements vscode.CodeLensProvider {
  ...
  public provideCodeLenses(
    ...
    return new Promise((resolve, reject) => {
      const allComponentsSymbols = getAllComponentsSymbol(
        this.referenceDocumentation,
        document
      );
      let resolves: vscode.CodeLens[] = [];
      const onlineDoc = _.map(
        allComponentsSymbols,
        (componentSymbol: vscode.SymbolInformation) => {
          const componentInfo = this.referenceDocumentation.getComponent(
            componentSymbol
          );
